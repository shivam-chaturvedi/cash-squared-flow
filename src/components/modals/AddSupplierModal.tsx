...
const AddSupplierModal = ({ open, onClose }: Props) => {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input type="email" placeholder="Email (optional)" className="w-full border border-input bg-background px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
          ...
          <button type="submit" className="w-full bg-primary text-primary-foreground py-2.5 text-sm font-medium hover:opacity-90">{tr.addSupplier}</button>
        </form>
      </DialogContent>
    </Dialog>
  );
