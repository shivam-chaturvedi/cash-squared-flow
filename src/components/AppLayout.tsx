...
const AppLayout = ({ children }: { children: ReactNode }) => {
  const { mode, setMode, language, setLanguage, userName } = useApp();
  ...
  return (
    <div className="min-h-screen flex bg-background">
      ...
      <aside className="hidden md:flex flex-col w-56 bg-card border-r border-border shrink-0">
        ...
        <div className="p-3 border-b border-border">
          <div className="flex bg-muted p-0.5">
            <button
              onClick={() => { setMode("business"); navigate("/"); }}
              className={`flex-1 py-1.5 text-xs font-medium transition ${mode === "business" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
            >
              {tr.business}
            </button>
            <button
              onClick={() => { setMode("personal"); navigate("/"); }}
              className={`flex-1 py-1.5 text-xs font-medium transition ${mode === "personal" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
            >
              {tr.personal}
            </button>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="md:hidden flex items-center justify-between px-4 py-3 bg-card border-b border-border">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex bg-muted p-0.5 text-xs">
              <button
                onClick={() => { setMode("business"); navigate("/"); }}
                className={`px-2 py-1 font-medium ${mode === "business" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
              >
                Biz
              </button>
              <button
                onClick={() => { setMode("personal"); navigate("/"); }}
                className={`px-2 py-1 font-medium ${mode === "personal" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
              >
                Me
              </button>
            </div>
        ...
        {/* FAB */}
        <button className="fixed bottom-20 right-4 md:bottom-6 md:right-6 w-12 h-12 bg-primary text-primary-foreground shadow-lg flex items-center justify-center hover:opacity-90 transition z-50">
