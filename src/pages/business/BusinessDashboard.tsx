  ...
  return (
    <div className="p-4 md:p-6 space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard label={tr.youllGive} value="₹45,200" variant="money-out" icon={<ArrowUpRight className="h-4 w-4 text-money-out" />} />
        <StatCard label={tr.youllGet} value="₹67,800" variant="money-in" icon={<ArrowDownLeft className="h-4 w-4 text-money-in" />} />
      </div>
      ...
      <AddTransactionModal open={showTransaction} onClose={() => setShowTransaction(false)} />
      <AddCustomerModal open={showCustomer} onClose={() => setShowCustomer(false)} />
      <AddExpenseModal open={showExpense} onClose={() => setShowExpense(false)} type="business" />
    </div>
  );
};
