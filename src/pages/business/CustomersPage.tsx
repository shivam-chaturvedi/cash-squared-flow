    ...
    <div className="h-full flex flex-col md:flex-row">
      ...
      <div className="flex-1 md:max-w-lg md:border-r border-border flex flex-col">
        ...
        <div className="flex items-center gap-6 px-4 py-3 border-b border-border bg-card text-sm">
          ...
          <button className="ml-auto text-primary text-xs font-medium border border-primary px-3 py-1 flex items-center gap-1">
            {tr.viewReport}
          </button>
        </div>
        ...
        <div className="p-4 space-y-3 bg-card border-b border-border">
          <div className="flex gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                placeholder={`${tr.search} ${tr.customers.toLowerCase()}`}
          ...
          <div className="flex gap-3 text-xs">
            <button className="flex items-center gap-1 border border-input px-3 py-1.5 text-muted-foreground hover:text-foreground">
              <SlidersHorizontal className="h-3 w-3" /> {tr.filterBy}
            </button>
            <button className="flex items-center gap-1 border border-input px-3 py-1.5 text-muted-foreground hover:text-foreground">
              <ArrowUpDown className="h-3 w-3" /> {tr.sortBy}
            </button>
          </div>
        </div>
            ...
            filtered.map((c) => (
              ...
              >
                ...
                <div className="text-right">
                  <p className={`text-sm font-semibold ${c.type === "get" ? "text-money-in" : "text-money-out"}`}>
                    ₹{Math.abs(c.balance).toLocaleString()}
                  </p>
                  <p className="text-[10px] text-muted-foreground">
                    {c.type === "get" ? tr.youllGet : tr.youllGive}
                  </p>
                </div>
              </button>
            ))
        ...
        <div className="flex gap-3 p-4 border-t border-border bg-card">
          <button className="flex-1 border border-input py-2.5 text-sm font-medium flex items-center justify-center gap-2 hover:bg-accent transition">
            <Upload className="h-4 w-4" /> {tr.bulkUpload}
          </button>
          <button className="flex-1 bg-primary text-primary-foreground py-2.5 text-sm font-medium flex items-center justify-center gap-2 hover:opacity-90 transition">
      ...
      <div className="hidden md:flex flex-1 items-center justify-center bg-background">
        {selectedCustomer ? (
          <div className="text-center animate-fade-in">
            <div className="w-16 h-16 bg-primary/10 flex items-center justify-center mx-auto mb-4 text-primary font-bold text-xl">
              {selectedCustomer.name.charAt(0)}
            </div>
            <h3 className="font-semibold text-lg">{selectedCustomer.name}</h3>
            <p className="text-sm text-muted-foreground">{selectedCustomer.phone}</p>
            <p className={`text-2xl font-bold mt-4 ${selectedCustomer.type === "get" ? "text-money-in" : "text-money-out"}`}>
              ₹{Math.abs(selectedCustomer.balance).toLocaleString()}
            </p>
            <p className="text-xs text-muted-foreground mt-1">
              {selectedCustomer.type === "get" ? tr.youllGet : tr.youllGive}
            </p>
