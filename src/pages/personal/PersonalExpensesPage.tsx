    ...
    <div className="p-4 md:p-6 space-y-4 animate-fade-in">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {["All", "Food", "Rent", "Travel", "Bills"].map((cat) => (
          <button key={cat} className="px-4 py-1.5 text-xs font-medium border border-input whitespace-nowrap hover:bg-accent transition first:bg-primary first:text-primary-foreground first:border-primary">{cat}</button>
        ))}
      </div>
      ...
      <div className="bg-card border border-border divide-y divide-border">
        {expenses.map((exp) => (
          <div key={exp.id} className="flex items-center gap-3 px-4 py-3">
            <div className="w-9 h-9 bg-muted flex items-center justify-center shrink-0"><exp.icon className="h-4 w-4 text-muted-foreground" /></div>
      ...
      <AddExpenseModal open={showAdd} onClose={() => setShowAdd(false)} type="personal" />
    </div>
  );
};
