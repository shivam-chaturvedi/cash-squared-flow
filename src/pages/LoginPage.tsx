    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <div className="w-full max-w-sm animate-fade-in">
        ...
        <div className="text-center mb-6">
          <img src="/logo.png" alt="Avail" className="w-16 h-16 mx-auto mb-3 object-contain" />
          <h1 className="text-2xl font-extrabold text-foreground">{tr.appName}</h1>
        ...
        <div className="bg-card border border-border p-5 shadow-sm">
          {!isSignup ? (
            ...
            <form onSubmit={handleSignupNext} className="space-y-3">
              ...
              <div className="flex gap-2">
                {signupStep > 1 && (
                  <button type="button" onClick={() => setSignupStep(signupStep - 1)}
          ...
          <button onClick={handleGoogle} className="w-full border border-input py-2.5 text-base font-medium flex items-center justify-center gap-2 hover:bg-accent transition mb-2">
          ...
          <button onClick={handleGuest} className="w-full text-base text-muted-foreground hover:text-foreground transition py-2">
            {tr.guestMode}
          </button>
        </div>
        ...
        <p className="text-center text-base text-muted-foreground mt-4">
          {isSignup ? (
            <>Already have an account? <button onClick={() => { setIsSignup(false); setSignupStep(1); }} className="text-primary font-semibold">{tr.login}</button></>
          ) : (
            <>Don't have an account? <button onClick={() => setIsSignup(true)} className="text-primary font-semibold">{tr.signup}</button></>
          )}
        </p>
      </div>
    </div>
